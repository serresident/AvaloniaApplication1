...
## ⚡ Производительность и Архитектурная Оптимизация (HMI/SCADA Performance)

Для обеспечения сверхнизкого времени отклика интерфейса и поддержки тысяч активных визуальных примитивов на одном экране без утечек памяти внедрены следующие архитектурные оптимизации:

### 1. Декаплинг разметки через Attached Properties (Присоединенные Свойства)
*   Кастомный панельный контейнер `DashboardPanel` больше не считывает свойства разметки (`Col`, `Row`, `SizeX`, `SizeY`) напрямую из `DataContext` / `WidgetViewModelBase`.
*   Вместо этого разметка позиционируется через **Attached Properties** (`v:DashboardPanel.Col` и т.д.), зарегистрированные на уровне панели.
*   Двусторонняя привязка к вьюмоделям задается декларативно в XAML (`DashboardView.axaml`) в стиле контейнера `ContentPresenter`.
*   Это позволяет отвязать рендеринг дашборда от конкретной структуры ViewModels и исключить утечки памяти при ручной подписке на события `PropertyChanged`.

### 2. Оптимизация телеметрии через Direct Properties (Прямые Свойства)
*   Свойства высокой частоты обновлений реального времени переведены из `StyledProperty` в **DirectProperty** (с непосредственным обращением к C#-полям):
    *   `Setpoint` и `Feedback` (уставка и обратная связь) в `ValveControl.cs`.
    *   `IsFilled` (присутствие потока/давления) в `PipeControl.cs`.
    *   `Values` (буфер точек тренда) в `TrendLineControl.cs`.
*   Это позволило полностью исключить аллокации памяти при постоянных обновлениях тегов Modbus/MQTT и повысить скорость перерисовки мнемосхемы.

### 3. Валидация и защита данных через Coercion (Коэрсия)
*   Ограничение диапазонов и валидация значений перенесены из отрисовки в метаданные свойств Avalonia (фаза `coerce`):
    *   `CellWidth`/`CellHeight` принудительно коэрсируются в диапазоне `[10.0, 500.0]`.
    *   `Thickness` (толщина труб и клапанов) коэрсируется в `[2.0, 50.0]`.
    *   `MinY`/`MaxY` для трендов коэрсируются в `[-10000.0, 10000.0]`.
    *   Уставки и аналоговая обратная связь ограничиваются безопасным диапазоном `[0.0, 100.0]` на этапе записи.
